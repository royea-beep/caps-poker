      ...
      <Pressable onPress={() => { if (onPress) { placeCard(); onPress(); } }} style={styles.pressableInner}>
        ...
        <View style={styles.header}>
          <View style={styles.headerLeft}>
            <Text style={styles.boardLabel}>{`לוח ${index + 1}`}</Text>
            {isArrangement && active && !boardFull && (
              <Text style={styles.activeLabel}>← מסדר</Text>
            )}
        ...
        {/* community badge removed - context obvious */}
...
const styles = StyleSheet.create({
  ...
  boardLabel: {
    ...
    ...Platform.select({
      ...
      android: { elevation: 2 },
      default: { boxShadow: '0 1px 3px rgba(0,0,0,0.5)' } as any,
    }),
  },
  rowLabel: {
