...
## TASK 7 — Enable Anonymous Sign-ins + Manual Linking in Supabase

Claude Code should do this via Supabase CLI:

```bash
cd C:\Projects\Caps

# Enable anonymous sign-ins
npx supabase --project-ref gxrpunvhjcrzqnitbqah auth config set --enable-anonymous-sign-ins true

# If CLI doesn't support this flag, use the Management API:
# 1. Get access token from Windows Credential Manager (key: "Supabase CLI:supabase")
# 2. PATCH https://api.supabase.com/v1/projects/gxrpunvhjcrzqnitbqah/config/auth
#    body: { "EXTERNAL_ANONYMOUS_USERS_ENABLED": true, "SECURITY_MANUAL_LINKING_ENABLED": true }
```

**If neither method works**, add this note to the audit output:
```
MANUAL: Enable Anonymous Sign-ins in Supabase Dashboard
  → Authentication → Settings → User Sign-ups → Anonymous Sign-Ins → ON
  → Also enable: Authentication → Settings → User Sign-ups → Manual Linking → ON
```

Manual Linking is required for `linkIdentity()` to convert anonymous → Google user.
...
Supabase client has AsyncStorage:            YES/NO
Anonymous Sign-ins enabled in Supabase:      YES/NO
Manual Linking enabled in Supabase:          YES/NO
