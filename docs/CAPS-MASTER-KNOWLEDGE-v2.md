...
> ⚠️ STALE HEADER AND IDENTITY (audit 2026-09-10: 27 stale claims). The app is 2.7.0 / build 515, not v1.9.3 / b104 / EAS #117; it is a multi-board game (2P=4, 3P=3, 4P=2 boards, Omaha-style per-board evaluation), not a single-board game; the repo is PUBLIC, not private; tests are 2,846/53 on main. Read the root `CLAUDE.md` first. *(banner added 2026-09-10 by the deep audit; body left as written)*
