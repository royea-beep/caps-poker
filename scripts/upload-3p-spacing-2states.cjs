...
(async () => {
  const files = ['3p-placement.png', '3p-ready.png'];
  for (const f of files) {
    const local = `C:/Projects/POKER/Caps/caps-ux-audit/2026-06-08/spacing/${f}`;
    const buf = fs.readFileSync(local);
    const storagePath = `caps-ux-audit/2026-06-08/spacing/${f}`;
    ...
    if (error) { console.log(`FAIL ${f}: ${error.message}`); process.exit(1); }
